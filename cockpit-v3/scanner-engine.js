...
function tradePlansFromAnalysis(analysis, direction) {
  ...
  const vwap = Number.isFinite(analysis.vwap) ? analysis.vwap : base - sign * atrAbs * 0.35;
  const support = Number.isFinite(analysis.support) ? analysis.support : base - atrAbs * 1.6;
  const resistance = Number.isFinite(analysis.resistance) ? analysis.resistance : base + atrAbs * 1.6;
  ...
  const entryFromStructure = (style) => {
    const pullbackAtr = {
      [TradeStyle.SCALP]: 0.25,
      [TradeStyle.INTRADAY]: 0.45,
      [TradeStyle.SWING]: 0.8,
    }[style] || 0.45;

    if (direction === Direction.LONG) {
      const planned = Math.max(support + atrAbs * 0.2, Math.min(vwap, base - atrAbs * pullbackAtr));
      return clampEntry(planned, base * 0.5, base * 0.998);
    }

    const planned = Math.min(resistance - atrAbs * 0.2, Math.max(vwap, base + atrAbs * pullbackAtr));
    return clampEntry(planned, base * 1.002, base * 1.5);
  };
  const plan = (style, risk, r1, r2, note, invalidation) => {
    const entry = entryFromStructure(style);
    return createTradePlan({
    style,
    direction,
    entry,
    stop: Math.max(0, entry * (1 - sign * risk)),
    target1: Math.max(0, entry * (1 + sign * risk * r1)),
    target2: Math.max(0, entry * (1 + sign * risk * r2)),
    invalidation,
    note,
  });
  };
...
function clampEntry(value, min, max) {
